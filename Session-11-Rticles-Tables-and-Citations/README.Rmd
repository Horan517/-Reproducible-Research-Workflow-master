...
The visual rmarkdown editor can also be used to create tables. This approach is not very useful when creating tables programmatically. So instead, there are several packages in `R` that are useful to create more sophisticated tables. My go-to package for this is {kableExtra}. The input for {kableExtra} is a data frame, which is convenient because likely most of the information you need to tabulate is there, or is the result of analysis thereof.
